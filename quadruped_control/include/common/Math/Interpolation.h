...
template <typename y_t, typename x_t>
y_t XYswingPos(y_t y0, y_t yf, x_t x) {
  static_assert(std::is_floating_point<x_t>::value,
                "must use floating point value");
  assert(x >= 0 && x <= 1); 

  y_t yDiff = yf - y0;
  x_t phase_PI = 2 * M_PI * x;
  x_t cofficient = (phase_PI - sin(phase_PI))/(2*M_PI);

  return y0 + yDiff * cofficient;
}

template <typename y_t, typename x_t>
y_t XYswingDeriv(y_t y0, y_t yf, x_t x) {
  static_assert(std::is_floating_point<x_t>::value,
                "must use floating point value");
  assert(x >= 0 && x <= 1); 
  ...
  y_t yDiff = yf - y0;
  x_t phase_PI = 2 * M_PI * x;
  x_t cofficient = (phase_PI - cos(phase_PI))/(2*M_PI);

  return yDiff * cofficient;
}
