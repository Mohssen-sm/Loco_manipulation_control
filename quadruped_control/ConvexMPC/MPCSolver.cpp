...
void MPCSolver::set_acclerations(double *_ang_accel, double *_lin_accel)
{
  mfp_to_flt(angular_accel, _ang_accel, 3);
  mfp_to_flt(linear_accel, _lin_accel, 3);
}

void MPCSolver::setup_adaptive(int horizon, int _state_num, float *_mu_2)
{
...
void MPCSolver::setup_LocoManipulation(int horizon, int _state_num)
{
  state_num = _state_num;
  resize_qp_mats(horizon);
}

void MPCSolver::set_manipulation_force(float *force)
{
  for (int i = 0; i < 3; i++)
...
void MPCSolver::solve_mpc()
{
  ...
  else if (state_num == 19)
  { // adaptive control
    x_0 << rpy(2), rpy(1), rpy(0), rs.p, rs.w, rs.v, 1.f, mu_2[0], mu_2[1], mu_2[2], mu_2[3], mu_2[4], mu_2[5];
  }
  else if (state_num == 15)
  { // loco-manipulation control
    x_0 << rpy(2), rpy(1), rpy(0), rs.p, rs.w, rs.v, 1.f,
