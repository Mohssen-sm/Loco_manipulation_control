...
void FSMState_ThreeFoot::run()
{
    ...
    if(counter > 1000)
    {
        ...
        //                                                 0, 10, 0,
        //                                                 0, 0, 10;
        _data->_legController->commands[0].kpJoint.diagonal()<< 60, 60, 60;
        _data->_legController->commands[0].kdJoint.diagonal()<< 2, 2, 2;
...
FSMStateName FSMState_ThreeFoot::checkTransition()
{
    if(_lowState->userCmd == UserCommand::L1_X){
        std::cout << "transition from 3foot to QP stand " << std::endl;
        return FSMStateName::QPSTAND;
    }
    else if(_lowState->userCmd == UserCommand::L2_B){
        std::cout << "transition from 3foot to passive " << std::endl;
